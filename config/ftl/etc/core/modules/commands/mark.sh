...
ftl::cmd::set_mark() {
        read -sn1
        # Use $REPLY (what the user typed), not $ftl_kbd_current_key (the
        # trigger key). The previous version checked ftl_kbd_current_key
        # which is the key that launched this command (e.g. 'm'), so
        # every mark was saved under the same key regardless of user input.
        if [[ -n "$REPLY" ]] ; then
                if [[ -d "${ftl_list_entries[$ftl_state_cursor_index]}" ]] ; then
                        ftl_mark_session_marks[$REPLY]="${ftl_list_entries[$ftl_state_cursor_index]}/$"
                else
                        ftl_mark_session_marks[$REPLY]="${ftl_list_entries[$ftl_state_cursor_index]}"
                fi
        fi
...
ftl::cmd::goto_mark() {
        read -n 1
        # Use $REPLY (user input), not $ftl_kbd_current_key (trigger key).
        if [[ -n "$REPLY" && -n "${ftl_mark_session_marks[$REPLY]:-}" ]] ; then
                local d
                d="$(dirname "${ftl_mark_session_marks[$REPLY]}")"
                ftl::list::change_dir "$d" "$(basename "${ftl_mark_session_marks[$REPLY]}")"
        else
                ftl::list::render
        fi
}
...
ftl::cmd::clear_persistent_marks() {
        ...
        # Use $REPLY (user's y/N answer), not $ftl_kbd_current_key (trigger key).
        if [[ $REPLY == y ]] ; then
