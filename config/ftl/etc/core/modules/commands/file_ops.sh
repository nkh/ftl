...
ftl::cmd::copy_to_prompted() {
        ...
        # Use $REPLY (user's destination input), not $ftl_kbd_current_key.
        if [[ -n "$REPLY" ]] ; then
                if ftl::sel::validate_existence ; then
                        _ftl::cmd::copy_or_move_with_tags copy "$REPLY"
                else
                        _ftl::cmd::copy_or_move copy "$REPLY" "${ftl_selection_current[@]}"
                fi
        fi
        ftl::list::change_dir
}
...
_ftl::cmd::dispatch_delete() {
        local pt="$1"
        # Use $REPLY (user's y/d/c answer), not $ftl_kbd_current_key.
        if [[ -n "$pt" ]] ; then
                if [[ $REPLY =~ y|d ]] ; then
...
ftl::cmd::create_file() {
        ...
        # Use $REPLY (user's filename input), not $ftl_kbd_current_key.
        if [[ -n "$REPLY" ]] ; then
                touch "$PWD/$REPLY"
        fi
        ftl::list::change_dir "$PWD" "$REPLY"
}
...
ftl::cmd::create_dir_no_cd() {
        ...
        # Use $REPLY (user's dirname input), not $ftl_kbd_current_key.
        if [[ -n "$REPLY" ]] ; then
                mkdir -p "$PWD/$REPLY"
        fi
        ftl::list::change_dir
...
ftl::cmd::symlink_selection() {
        ...
        # Use $REPLY (user's y/N answer), not $ftl_kbd_current_key.
        if [[ $REPLY == y ]] ; then
                ftl::sel::clear_all
                local f
...
ftl::cmd::copy_to_preset() {
        read -n 1
        # Use $REPLY (user's preset key), not $ftl_kbd_current_key.
        if [[ -n "$REPLY" && -n "${ftl_cfg_preset_destinations[$REPLY]:-}" ]] ; then
                ftl::cmd::copy_selection_here "${ftl_cfg_preset_destinations[$REPLY]}"
        fi
}
...
ftl::cmd::move_to_preset() {
        ...
        # Use $REPLY (user's preset key), not $ftl_kbd_current_key.
        if [[ -n "$REPLY" && -n "${ftl_cfg_preset_destinations[$REPLY]:-}" ]] ; then
                ftl::cmd::move_selection_here "${ftl_cfg_preset_destinations[$REPLY]}"
        fi
}
...
ftl::cmd::preview_with_command() {
        ...
        ftl::list::render
        # Use $REPLY (user's command input), not $ftl_kbd_current_key.
        if [[ -n "$REPLY" ]] ; then
                ftl::prev::clear
                ftl::pane::split "$REPLY '$ftl_state_current_basename' ; read -sn1"
        else
                ftl::list::change_dir
        fi
}
