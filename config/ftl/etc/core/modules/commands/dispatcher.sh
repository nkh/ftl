...
ftl::cmd::dispatch_command() {
        # Empty or whitespace-only command — cancel
        [[ -z "${1// }" ]] && return 1
        [[ "$1" == 0 ]] && return 1
        ...
        if [[ "$1" =~ ^[1-9][0-9]*$ ]] ; then
                local idx=$(( $1 - 1 ))
                # Bounds check: under set -u, accessing an out-of-range
                # array index crashes. Guard before dereferencing.
                (( idx < 0 || idx >= ftl_list_entry_count )) && return 1
                local entry="${ftl_list_entries[$idx]}"
                # Set the cursor INDEX (numeric), not the entry PATH.
                # The previous version assigned the path string to
                # ftl_state_cursor_index, which broke all downstream
                # arithmetic on the index.
                ftl_state_cursor_index=$idx
                if [[ -d "$entry" ]] ; then
                        ftl::list::change_dir "$entry"
                else
                        ftl::list::render "$idx"
                fi
                return 1
        ...
        # Guard against empty cmd_parts (e.g. input was only whitespace
        # that parse_parts reduced to nothing). Under set -u, accessing
        # cmd_parts[0] when unset crashes.
        [[ ${#cmd_parts[@]} -eq 0 ]] && return 1
