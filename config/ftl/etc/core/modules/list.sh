...
#   _ftl::list::scan_custom_source — scan from a custom source (fzf-listen) (was: get_custom_entries)
...
# Get virtual dirs (placeholder for the virtual_entries plugin).
ftl::plugin::virtual::get_virtual_dirs() {
    (( ${#ftl_plugin_vdirs[@]} )) && printf "0\t0\t%s\n" "${!ftl_plugin_vdirs[@]}"
}

# Inject virtual entries (placeholder, calls the plugin callbacks).
ftl::plugin::virtual::inject_entries() {
    ftl_plugin_vfiles=()
    ftl_plugin_vdirs=()
    if (( ftl_plugin_virtual_enabled )) ; then
        local v
        while read -r v ; do ftl_plugin_vdirs[$v]=1 ; done < <(ftl::plugin::virtual::get_dirs_callback "$@")
        while read -r v ; do ftl_plugin_vfiles[$v]=1 ; done < <(ftl::plugin::virtual::get_files_callback "$@")
    fi
}
