...
_ftl::plugin::inline_rename::commit() {
        ...
        # Attempt the rename. If mv fails (e.g. permission denied), do
        # NOT record history or clear the draft — surface the error and
        # let the user retry with a different name.
        if ! mv -- "$ftl_inline_rename_original_path" "$target" 2>/dev/null ; then
                ftl_state_inline_rename_error="rename failed: $ftl_inline_rename_draft"
                _ftl::plugin::inline_rename::render_draft
                ftl_state_inline_rename_error=
                return
        fi
...
_ftl::plugin::inline_rename::abort() {
        ...
        ftl_inline_rename_draft_cursor=0
        ftl_inline_rename_is_label=0
        # Return to outer mode (active=1), not inactive (active=0).
        # abort is called from inner mode (ESCAPE/RETURN) and should
        # return to outer mode so the user can start a new rename.
        # exit() is the function that fully leaves inline rename (active=0).
        ftl_inline_rename_active=1
        ftl::list::render
}
...
_ftl::plugin::inline_rename::sequential() {
        ...
        for target in "${ftl_inline_rename_bulk_targets[@]}" ; do
                ...
                # Extract extension only for non-dotfile names.
                # A leading-dot file like ".bashrc" is a dotfile, not
                # "name=.bashrc, ext=bashrc" — the previous logic matched
                # *.* on the leading dot and treated the whole name as
                # having an extension. We require a dot AFTER the first
                # character to count as having an extension.
                if [[ "$name" == *.* && "$name" != .* ]] ; then
                        ext=".${name##*.}"
                elif [[ "$name" == .*.* ]] ; then
                        # dotfile with extension, e.g. ".bashrc.bak"
                        ext=".${name##*.}"
                fi
