...
# This is a read-only operation and must NOT increment the revision
# counter — doing so causes spurious pane-sync cycles.
ftl::sel::build_class_index() {
...
ftl::sel::load_from_file() {
        # Use `set` (not `flip`) so loading the same file twice is
        # idempotent. The previous implementation used `flip`, which
        # toggled tags off on the second load.
        local p
        while read -r p ; do
                [[ -z "$p" ]] && continue
...
ftl::sel::adjust_total_size() {
        # Guard against non-existent files: stat returns empty on failure,
        # and `(( x +=  ))` (empty operand) is a syntax error under set -u.
        # We also guard against the total going negative (which could
        # happen if a file is deleted between add and subtract).
        local size
        size=$(stat --printf="%s" "$2" 2>/dev/null) || return 0
        [[ -n "$size" ]] || return 0
        if [[ $1 == "+" ]] ; then
                (( ftl_selection_total_bytes += size ))
                true
        else
                (( ftl_selection_total_bytes -= size ))
                # Guard against negative total
                (( ftl_selection_total_bytes < 0 )) && ftl_selection_total_bytes=0
                true
        fi
}
