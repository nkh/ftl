...
case "${REPLY: -1}" in
	...
	${C[find_fzf_all]})	exec 2>&9 ; tcpreview ; fzf_go "$(fd -HI -E'.git/*' | fzf_vvip $fzfp_opt)" ; exec 2>"$fs/error_log" ;;
	${C[find_fzf_dirs]})	exec 2>&9 ; tcpreview ; fzf_go "$(fd -td -I -L | fzf_vvip $fzfp_opt)" ; exec 2>"$fs/error_log" ;;
	...
	${C[preview_show]})	[[ $e =~ $mfilter ]] && { ($PREVIEW_SHOW "${selection[@]}" & ) ; tags_clear ; list ; } ;;
...
quit2()     { inotify_k ; stty echo ; [[ $pfs == $fs ]] && tbcolor $TBCOLORS ; player_k ; kill $w3iproc &>/dev/null ; refresh "\e[?25h\e[?1049l" ; cdfl ; }
