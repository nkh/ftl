...
case "${REPLY: -1}" in
	...
	H      ) prompt 'clear global history? [y|N]' -sn1 && [[ $REPLY == y ]] && rm $ftl_root/history 2>&- ; list ;;
	...
	\:     ) prompt ':' ; printf -v line '%d' "$REPLY" 2>&- ; list $((line > 0 ? (line -1) : 0)) ;; 
	...
	đ      ) p=~/.config/ftl/external_filters ; file=$(cd $p ; fd | fzf) ; [[ $file ]] && . $p/$file ; cdir ;;
