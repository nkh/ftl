...
case "${REPLY: -1}" in
	...
	${K[b]}) fzf_go "$(listing | fzf --ansi --info=inline --layout=reverse | { ((show_line)) && { perl -pe 's/^.*[0-9]+\ +//g' ; } || /bin/cat ; })" ;;
...
listing()   { for((i=$top ; i <= ((bottom = top + lines - 1, bottom < 0 ? 0 : bottom)) ; i++)) ; do echo "${files_color[i]}" ; done ; }
